...
export function SiteFooter() {
  ...
  const introWhere =
    `${visitRestaurant.address}, ${visitRestaurant.cityLine}` +
  ...
  return (
    <footer
      id="contact"
      className="scroll-mt-[calc(var(--nav-h)+16px)] border-t border-white/10 bg-charcoal py-16"
    >
      <div className="mx-auto flex max-w-[1200px] flex-col gap-10 px-5 sm:flex-row sm:justify-between sm:px-8">
        <div className="max-w-sm space-y-4">
          ...
          <p className="text-sm leading-relaxed text-cream/70">
            {CONTACT.socialHandle} — La Hamburguesa Loca. Auténtico Sazón Mexicano. Restaurant and
            food truck: {introWhere}.
          </p>
        ...
        <div className="grid gap-8 sm:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-6 text-xs text-cream/50">
              Menu updates from Google Sheets (see README, section Restaurant Menu Updates). Order
              requests are sent to the team; final price and pickup time are confirmed by phone or
              email.
            </p>
          </div>
        </div>
